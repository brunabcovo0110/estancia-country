...
Importe o repositório na Vercel e clique em **Deploy** — não é preciso mudar nada (Root Directory `./`).
As configurações ficam em `vercel.json`. Para o formulário enviar de verdade, adicione `VITE_FORM_ENDPOINT`
em *Settings → Environment Variables* (veja `.env.example`) e faça um novo deploy.
